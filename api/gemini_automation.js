...
// --- Replaces askPerplexity with Gemini ---
async function askGemini(prompt) {
  log("🧠 Asking Gemini 2.5 Flash...");
  const res = await fetch("https://tableautomation.vercel.app/api/gemini_supabase", {
    method: "POST",
...
classSelect.addEventListener('change', async () => {
  ...
  try {
    ...
    const subjects = JSON.parse(raw.replace(/```json|```/g, '').trim());
...
subjectSelect.addEventListener('change', async () => {
  ...
  try {
    const data = await askGemini(prompt);
    const raw = data?.candidates?.[0]?.content?.parts?.[0]?.text || "[]";
    const chapters = JSON.parse(raw.replace(/```json|```/g, '').trim());
...
`;
  ...
  try {
    ...
    const csv = raw.replace(/```csv|```/g, '').trim();
...
// --- Chapter selection enables generate ---
